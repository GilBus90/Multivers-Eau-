...
function usePiAuth(){
  ...
  useEffect(()=>{
    ...
    window.Pi.authenticate(["username","payments"],async(inc)=>{
      try{await fetch(`${API_URL}/payments/incomplete`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({paymentId:inc.identifier})});}catch(e){}
    }).then(async(auth)=>{
      setUser(auth.user);
      try{
        const res=await fetch(`${API_URL}/users/role`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({accessToken:auth.accessToken,username:auth.user.username})});
...
function LandingPage({onRole,oracle,lang,setLang}){
  ...
  const t=T[lang];
  const ROLES=[
    {id:"client", icon:"🛍️",color:C.client, fr:"Commander",          en:"Order",           dfr:"Achetez votre eau en Pi, livrée chez vous",             den:"Buy your water in Pi, delivered to you"},
    {id:"livreur",icon:"🏍️",color:C.livreur,fr:"Je suis Livreur",    en:"I am a Driver",   dfr:"Gérez vos courses et vos gains Pi",                     den:"Manage your deliveries and Pi earnings"},
    {id:"relais", icon:"🏪",color:C.relais, fr:"Je suis un Relais",  en:"I am a Relay",    dfr:"Gérez votre dépôt, vos stocks, vos livreurs",           den:"Manage your depot, stock, drivers"},
    {id:"admin",  icon:"⚙️",color:C.admin,  fr:"Super Admin",        en:"Super Admin",     dfr:"Pilotage national · Oracle · Commissions",              den:"National control · Oracle · Commissions"},
  ];
  return(
    <AppWrap>
      {/* Hero */}
      <div style={{background:"radial-gradient(ellipse 140% 100% at 50% 0%,#001A5E 0%,#060912 65%)",padding:"44px 20px 30px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        {[200,140,80].map(s=><div key={s} style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:s,height:s,borderRadius:"50%",border:`1px solid rgba(0,102,255,${s===80?.14:s===140?.09:.05})`,pointerEvents:"none"}}/>)}
        {/* Lang toggle */}
        <div style={{position:"absolute",top:16,right:16}}>
          <button onClick={()=>setLang(l=>l==="fr"?"en":"fr")} style={{background:"rgba(255,255,255,.1)",border:"1px solid rgba(255,255,255,.2)",borderRadius:20,padding:"5px 14px",color:"#fff",fontSize:12,fontWeight:700,cursor:"pointer"}}>
            {t.lang}
          </button>
        </div>
        <div style={{position:"relative"}}>
          <div style={{display:"inline-block",background:"linear-gradient(135deg,#0033A8,#0066FF)",borderRadius:"50%",width:70,height:70,fontSize:34,lineHeight:"70px",marginBottom:14,boxShadow:"0 0 40px #0066FF44"}}>💧</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:30,fontWeight:900,background:"linear-gradient(135deg,#fff 0%,#99C2FF 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:5}}>{t.appName}</div>
          <div style={{fontSize:13,color:C.sub,marginBottom:18}}>{t.tagline}</div>
          <OracleBadge oracle={oracle} lang={lang}/>
        </div>
      </div>
      {/* Rôles */}
      <div style={{padding:"22px 18px"}}>
        <div style={{fontSize:11,color:C.muted,fontWeight:700,letterSpacing:1,marginBottom:12,textAlign:"center"}}>{lang==="fr"?"QUI ÊTES-VOUS ?":"WHO ARE YOU?"}</div>
        {ROLES.map((r,i)=>(
          <div key={r.id} className="fu" style={{animationDelay:`${i*.07}s`}} onClick={()=>onRole(r.id)}>
            <div style={{display:"flex",alignItems:"center",gap:14,padding:"15px 18px",marginBottom:10,borderRadius:18,cursor:"pointer",background:C.card,border:`1.5px solid ${C.border}`,transition:"all .15s"}}
              onMouseEnter={e=>{e.currentTarget.style.borderColor=r.color;e.currentTarget.style.background=`${r.color}10`;}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.background=C.card;}}>
              <div style={{width:50,height:50,borderRadius:14,flexShrink:0,background:`${r.color}22`,border:`1px solid ${r.color}44`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{r.icon}</div>
              <div style={{flex:1}}>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:15,color:C.text}}>{lang==="fr"?r.fr:r.en}</div>
                <div style={{fontSize:12,color:C.sub,marginTop:2}}>{lang==="fr"?r.dfr:r.den}</div>
              </div>
              <div style={{color:r.color,fontSize:18}}>→</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{padding:"0 18px 32px"}}>
        <div onClick={()=>setShowAcad(true)} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",borderRadius:16,cursor:"pointer",background:"linear-gradient(135deg,#0A1828,#0F2040)",border:"1px solid #1E3A6E"}}>
          <span style={{fontSize:26}}>🎓</span>
          <div>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,color:C.admin,fontSize:14}}>{lang==="fr"?"Académie Pi Network":"Pi Network Academy"}</div>
            <div style={{fontSize:12,color:C.sub}}>{lang==="fr"?"Code invitation :":"Invitation code:"} <strong style={{color:C.admin}}>{CODE_INVITATION}</strong></div>
          </div>
          <span style={{marginLeft:"auto",color:C.admin,fontSize:18}}>→</span>
        </div>
        <div style={{textAlign:"center",marginTop:12,fontSize:11,color:C.muted}}>
          {lang==="fr"?"Prix indexé Oracle CoinGecko · 1π =":"CoinGecko Oracle price · 1π ="} {fmt(oracle.rate)} FCFA
        </div>
      </div>
...
function ClientApp({oracle,stocks,dec,lang,setLang,onBack}){
  ...
  const payerAvecPi=async()=>{
    if(!window.Pi){show(lang==="fr"?"Ouvrez dans Pi Browser !":"Open in Pi Browser!",C.red);return;}

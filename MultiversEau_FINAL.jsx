...
// Oracle CoinGecko — refresh 60s + countdown + multi-fallback
const ORACLE_INTERVAL=60;
function useOracle(){
  ...
  const[countdown,setCountdown]=useState(ORACLE_INTERVAL);
  const cdRef=useRef(ORACLE_INTERVAL);
  const fetchingRef=useRef(false);

  const addHistory=(xof)=>{
    const t=new Date().toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"});
    setHistory(h=>[...h.slice(-29),{t,v:xof}]);
  };

  const sync=useCallback(async()=>{
    if(modeManuel||fetchingRef.current)return;
    fetchingRef.current=true;
    setStatus("loading");
    cdRef.current=ORACLE_INTERVAL;
    setCountdown(ORACLE_INTERVAL);

    // Tentative 1 — CoinGecko XOF direct
    try{
      const r=await fetch(
        "https://api.coingecko.com/api/v3/simple/price?ids=pi-network&vs_currencies=xof,usd&precision=2",
        {signal:AbortSignal.timeout(8000),cache:"no-cache"}
      );
      if(r.ok){
        const d=await r.json();
        const xof=d?.["pi-network"]?.xof;
        const usd=d?.["pi-network"]?.usd;
        if(xof>0){
          setRate(xof);setStatus("live");setTime(new Date());addHistory(xof);
          fetchingRef.current=false;return;
        }
        // Fallback XOF≈USD×600 (FCFA lié EUR, ~600/USD)
        if(usd>0){
          const xofEst=Math.round(usd*600);
          setRate(xofEst);setStatus("live");setTime(new Date());addHistory(xofEst);
          fetchingRef.current=false;return;
        }
      }
    }catch(e){}

    // Tentative 2 — CoinGecko USD seulement
    try{
      ...
      if(r2.ok){
        ...
        const usd2=d2?.["pi-network"]?.usd;
        if(usd2>0){
          const xofEst=Math.round(usd2*600);
          setRate(xofEst);setStatus("live");setTime(new Date());addHistory(xofEst);
          fetchingRef.current=false;return;
        }
      }
  ...
  // Countdown 1s + auto-sync toutes les 60s
  useEffect(()=>{
    if(modeManuel)return;
    sync();
    const id=setInterval(()=>{
      cdRef.current=cdRef.current<=1?ORACLE_INTERVAL:cdRef.current-1;
      setCountdown(cdRef.current);
      if(cdRef.current===ORACLE_INTERVAL)sync();
    },1000);
    return()=>clearInterval(id);
  },[sync,modeManuel]);

  const appliquerManuel=(v)=>{const n=parseFloat(v);if(n>0){setRate(n);setModeManuel(true);setTime(new Date());addHistory(n);}};
  const desactiverManuel=()=>{setModeManuel(false);sync();};
  return{rate,status,time,history,sync,countdown,modeManuel,rateManuel,setRateManuel,appliquerManuel,desactiverManuel};
}
...
const AppWrap=({children,mw})=>{
  const w=useWindowWidth();
  // Mobile <640px → 460px | Tablet 640-1024px → 600px | Desktop >1024px → 680px
  const maxW=mw||(w>=1024?680:w>=640?600:460);
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Nunito',sans-serif",color:C.text,maxWidth:maxW,margin:"0 auto",width:"100%",transition:"max-width .2s"}}>
...
function OracleBadge({oracle,lang,compact}){
  ...
  return(
    <div onClick={oracle.sync} title="Cliquer pour forcer la synchronisation" style={{display:"flex",alignItems:"center",gap:7,background:"rgba(255,255,255,.06)",borderRadius:20,padding:compact?"5px 12px":"8px 16px",cursor:"pointer",border:`1px solid ${col}33`}}>
      ...
      <div>
        {!compact&&<div style={{fontSize:8,color:col,fontWeight:800,letterSpacing:1}}>
          {oracle.modeManuel?"TAUX FIXE":oracle.status==="loading"?"SYNCING…":oracle.status==="live"?"ORACLE LIVE":"FALLBACK"}
        </div>}
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:compact?12:15,fontWeight:900,color:"#fff"}}>1π = {fmt(oracle.rate)} F</div>
        {!compact&&!oracle.modeManuel&&(
          <div style={{fontSize:8,color:C.muted,marginTop:1}}>↻ {lang==="fr"?"dans":"in"} {oracle.countdown}s</div>
        )}
      </div>
    </div>
  );
...
function TradingViewChart({height=320}){
  // iframe embed — fiable dans tous les environnements (pas de script injection bloqué par CSP)
  const src="https://www.tradingview.com/widgetembed/?frameElementId=mv_piusdt&symbol=MEXC%3APIUSDT&interval=60&theme=dark&style=1&locale=fr&toolbar_bg=%23060912&enable_publishing=false&hide_top_toolbar=false&hide_legend=false&save_image=false&allow_symbol_change=false&backgroundColor=%23060912";
  return(
    <div>
      ...
      <div style={{height,borderRadius:"0 0 12px 12px",overflow:"hidden",border:`1px solid ${C.border}`,borderTop:"none",background:C.surf}}>
        <iframe
          src={src}
          width="100%"
          height="100%"
...
function BottomNav({tabs,active,onSelect,color}){
  const w=useWindowWidth();
  const maxW=w>=1024?680:w>=640?600:460;
  return(
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:maxW,background:C.surf,borderTop:`1px solid ${C.border}`,display:"flex",zIndex:500,paddingBottom:"env(safe-area-inset-bottom)"}}>
      {tabs.map(t=>(
...
// ════════════════════════════════════════════════════════════════════════════
// ONBOARDING FLOW + mémoire rôle
function useRoleMemory(){
  const KEY="mve_role_v1";
  const getSaved=()=>{try{return localStorage.getItem(KEY);}catch{return null;}};
  const save=(r)=>{try{localStorage.setItem(KEY,r);}catch{}};
  const clear=()=>{try{localStorage.removeItem(KEY);}catch{}};
  return{getSaved,save,clear};
}

function OnboardingFlow({onRole,oracle,lang,setLang}){
  const[step,setStep]=useState("splash");
  const[showAcad,setShowAcad]=useState(false);
  const goTo=(s)=>setTimeout(()=>setStep(s),180);
  const choisir=(r)=>onRole(r);

  if(step==="splash")return(
    <AppWrap>
      <style>{`
        @keyframes dropFall{0%{transform:translateY(-60px);opacity:0}60%{transform:translateY(8px)}100%{transform:translateY(0);opacity:1}}
      `}</style>
      <div style={{background:"linear-gradient(180deg,#020818 0%,#030D2E 50%,#050A1A 100%)",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"0 24px",position:"relative",overflow:"hidden"}}>
        {[320,220,140].map(s=><div key={s} style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:s,height:s,borderRadius:"50%",border:`1px solid rgba(0,120,255,${s===140?0.1:s===220?0.07:0.04})`,pointerEvents:"none"}}/>)}
        <div style={{width:100,height:100,borderRadius:"50%",background:"linear-gradient(135deg,#0033A8,#0066FF)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:46,marginBottom:28,boxShadow:"0 0 60px rgba(0,102,255,0.5)",animation:"dropFall 0.7s ease both"}}>💧</div>
        <div className="fu" style={{fontFamily:"'Syne',sans-serif",fontSize:34,fontWeight:900,textAlign:"center",background:"linear-gradient(135deg,#FFFFFF 0%,#7AB8FF 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",marginBottom:10}}>Multivers'Eau</div>
        <div className="fu" style={{fontSize:14,color:"rgba(255,255,255,0.5)",textAlign:"center",lineHeight:1.7,marginBottom:16,maxWidth:280}}>
          {lang==="fr"?"La première Super-App eau minérale Pi Network au Togo":"The first Pi Network mineral water Super-App in Togo"}
        </div>
        <div className="fu" style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.06)",borderRadius:24,padding:"7px 16px",marginBottom:44,border:"1px solid rgba(0,102,255,0.25)"}}>
          ...
          <span style={{fontSize:13,fontWeight:700,color:"#fff"}}>1π = {fmt(oracle.rate)} FCFA</span>
          <span style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>CoinGecko</span>
        </div>
        <button onClick={()=>goTo("choix")} style={{width:"100%",maxWidth:320,padding:"18px 0",background:"linear-gradient(135deg,#0044CC,#0066FF)",border:"none",borderRadius:18,color:"#fff",fontFamily:"'Syne',sans-serif",fontWeight:900,fontSize:17,cursor:"pointer",boxShadow:"0 8px 30px rgba(0,102,255,0.45)"}}>
          {lang==="fr"?"Découvrir →":"Discover →"}
        </button>
        <button onClick={()=>setLang(l=>l==="fr"?"en":"fr")} style={{marginTop:16,background:"transparent",border:"1px solid rgba(255,255,255,0.12)",borderRadius:16,padding:"6px 16px",color:"rgba(255,255,255,0.4)",fontSize:12,cursor:"pointer"}}>
  ...
  if(step==="choix")return(
    <AppWrap>
      <div style={{background:"linear-gradient(180deg,#020818,#040C22)",minHeight:"100vh",padding:"48px 22px 32px"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{fontSize:32,marginBottom:10}}>💧</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:900,color:"#fff",marginBottom:8}}>
          ...
          <div style={{fontSize:13,color:"rgba(255,255,255,0.4)",lineHeight:1.6,maxWidth:300,margin:"0 auto"}}>
            {lang==="fr"?"Multivers'Eau s'adapte à votre rôle dans l'écosystème":"Multivers'Eau adapts to your role in the ecosystem"}
          </div>
        </div>
        {/* COMMANDER */}
        <div onClick={()=>choisir("client")} style={{background:"linear-gradient(135deg,#001A5E,#002D99)",border:"1.5px solid rgba(0,102,255,0.4)",borderRadius:22,padding:"24px 22px",cursor:"pointer",marginBottom:14,boxShadow:"0 8px 32px rgba(0,50,180,0.35)"}}>
          <div style={{fontSize:38,marginBottom:10}}>🛍️</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:900,color:"#fff",marginBottom:8}}>{lang==="fr"?"Commander de l'eau":"Order water"}</div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.55)",lineHeight:1.6,marginBottom:14}}>
            {lang==="fr"?"Recevez Voltic, Cristal ou Eau Vitale chez vous. Payez en Pi, livraison rapide dans votre zone.":"Get Voltic, Cristal or Eau Vitale delivered home. Pay in Pi, fast delivery in your zone."}
          </div>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(0,102,255,0.25)",borderRadius:20,padding:"6px 14px"}}>
            <span style={{fontSize:12,color:"#7AB8FF",fontWeight:700}}>{lang==="fr"?"Démarrer →":"Start →"}</span>
          </div>
        </div>
        {/* REJOINDRE */}
        <div onClick={()=>goTo("distribution")} style={{background:"linear-gradient(135deg,#1A0A00,#2D1500)",border:"1.5px solid rgba(255,107,0,0.35)",borderRadius:22,padding:"24px 22px",cursor:"pointer",marginBottom:14,boxShadow:"0 8px 32px rgba(180,60,0,0.3)"}}>
          <div style={{fontSize:38,marginBottom:10}}>🤝</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:900,color:"#fff",marginBottom:8}}>{lang==="fr"?"Rejoindre le réseau":"Join the network"}</div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.55)",lineHeight:1.6,marginBottom:14}}>
            {lang==="fr"?"Devenez partenaire de distribution. Gagnez des Pi en livrant ou en gérant un dépôt d'eau minérale.":"Become a distribution partner. Earn Pi by delivering or managing a mineral water depot."}
          </div>
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(255,107,0,0.2)",borderRadius:20,padding:"6px 14px"}}>
            <span style={{fontSize:12,color:"#FF9A4D",fontWeight:700}}>{lang==="fr"?"En savoir plus →":"Learn more →"}</span>
          </div>
        </div>
        {/* ACADÉMIE PI */}
        <div onClick={()=>setShowAcad(true)} style={{padding:"14px 18px",background:"rgba(0,212,255,0.06)",border:"1px solid rgba(0,212,255,0.2)",borderRadius:16,cursor:"pointer",display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
          <span style={{fontSize:26}}>🎓</span>
          <div>
            <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,color:"#00D4FF",fontSize:14,marginBottom:3}}>{lang==="fr"?"C'est quoi Pi Network ?":"What is Pi Network?"}</div>
            <div style={{fontSize:12,color:"rgba(255,255,255,0.4)"}}>{lang==="fr"?"Code invitation :":"Invitation code:"} <strong style={{color:"#00D4FF"}}>flashman90</strong></div>
          </div>
          <span style={{marginLeft:"auto",color:"#00D4FF",fontSize:18}}>→</span>
        </div>
        <button onClick={()=>setLang(l=>l==="fr"?"en":"fr")} style={{width:"100%",padding:"8px",background:"transparent",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,color:"rgba(255,255,255,0.3)",fontSize:12,cursor:"pointer",marginBottom:8}}>
          {lang==="fr"?"🇬🇧 English":"🇫🇷 Français"}
        </button>
        <button onClick={()=>goTo("splash")} style={{width:"100%",padding:"8px",background:"transparent",border:"none",color:"rgba(255,255,255,0.2)",fontSize:12,cursor:"pointer"}}>← {lang==="fr"?"Retour":"Back"}</button>
      </div>
      {showAcad&&<AcademiePi lang={lang} onClose={()=>setShowAcad(false)}/>}
    </AppWrap>
  );

  if(step==="distribution")return(
    <AppWrap>
      <div style={{background:"linear-gradient(180deg,#020818,#04100A)",minHeight:"100vh",padding:"48px 22px 32px"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{fontSize:32,marginBottom:10}}>🤝</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:900,color:"#fff",marginBottom:8}}>{lang==="fr"?"Quel est votre rôle ?":"What is your role?"}</div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.4)",lineHeight:1.6}}>{lang==="fr"?"Choisissez votre place dans l'écosystème":"Choose your place in the ecosystem"}</div>
        </div>
        {/* LIVREUR */}
        <div onClick={()=>choisir("livreur")} style={{background:"linear-gradient(135deg,#1A0900,#2D1200)",border:"1.5px solid rgba(255,107,0,0.4)",borderRadius:20,padding:"22px 20px",cursor:"pointer",marginBottom:14}}>
          <div style={{fontSize:34,marginBottom:10}}>🏍️</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:900,color:"#fff",marginBottom:6}}>{lang==="fr"?"Je suis Livreur":"I am a Delivery Driver"}</div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",lineHeight:1.5,marginBottom:12}}>{lang==="fr"?"Moto, tricycle ou voiture ? Acceptez des courses et gagnez des Pi à chaque livraison.":"Motorcycle, tricycle or car? Accept deliveries and earn Pi for each one."}</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {(lang==="fr"?["🏍️ Moto","🛺 Tricycle","🚗 Voiture","💰 Pi instantanés"]:["🏍️ Motorcycle","🛺 Tricycle","🚗 Car","💰 Instant Pi"]).map(tag=>(
              <span key={tag} style={{fontSize:10,fontWeight:700,padding:"3px 10px",borderRadius:20,background:"rgba(255,107,0,0.18)",color:"#FF9A4D"}}>{tag}</span>
            ))}
          </div>
        </div>
        {/* RELAIS */}
        <div onClick={()=>choisir("relais")} style={{background:"linear-gradient(135deg,#1A1000,#2D1C00)",border:"1.5px solid rgba(245,158,11,0.4)",borderRadius:20,padding:"22px 20px",cursor:"pointer",marginBottom:14}}>
          <div style={{fontSize:34,marginBottom:10}}>🏪</div>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:900,color:"#fff",marginBottom:6}}>{lang==="fr"?"Je suis un Relais (Dépôt)":"I am a Relay (Depot)"}</div>
          <div style={{fontSize:13,color:"rgba(255,255,255,0.5)",lineHeight:1.5,marginBottom:12}}>{lang==="fr"?"Local de stockage ? Devenez un point de distribution officiel dans votre région.":"Storage space? Become an official distribution point in your region."}</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
            {(lang==="fr"?["📦 Stocks","🏍️ Livreurs","💼 Fiscal","📊 Dashboard"]:["📦 Stock","🏍️ Drivers","💼 Tax","📊 Dashboard"]).map(tag=>(
              <span key={tag} style={{fontSize:10,fontWeight:700,padding:"3px 10px",borderRadius:20,background:"rgba(245,158,11,0.18)",color:"#F59E0B"}}>{tag}</span>
            ))}
          </div>
        </div>
        <div style={{padding:"12px 16px",borderRadius:12,background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.06)",fontSize:12,color:"rgba(255,255,255,0.3)",lineHeight:1.6,marginBottom:14}}>
          ℹ️ {lang==="fr"?"Votre candidature sera examinée par votre relais régional puis validée par le Super Admin.":"Your application will be reviewed by your regional relay then validated by the Super Admin."}
        </div>
        <button onClick={()=>goTo("choix")} style={{width:"100%",padding:"10px",background:"transparent",border:"none",color:"rgba(255,255,255,0.25)",fontSize:12,cursor:"pointer"}}>← {lang==="fr"?"Retour":"Back"}</button>
      </div>
    </AppWrap>
  );
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Nunito',sans-serif",color:C.text}}>
      ...
      <div style={{display:"flex",minHeight:"100vh"}}>
        {/* Sidebar — visible desktop, caché mobile */}
        {!isMobileAdmin&&<div style={{width:190,background:C.surf,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",padding:"18px 10px",position:"sticky",top:0,height:"100vh",flexShrink:0,overflow:"hidden"}}>
        ...
        {/* Nav mobile admin */}
        {isMobileAdmin&&(
          <div style={{position:"fixed",top:0,left:0,right:0,zIndex:500,background:C.surf,borderBottom:`1px solid ${C.border}`,display:"flex",overflowX:"auto",scrollbarWidth:"none"}}>
            <div style={{padding:"8px 12px",display:"flex",alignItems:"center",gap:6,flexShrink:0,borderRight:`1px solid ${C.border}`}}>
              <span style={{fontSize:12,fontWeight:900,color:COL}}>💧</span>
            </div>
            {NAV.map(n=>(
              <button key={n.id} onClick={()=>setSection(n.id)} style={{flexShrink:0,padding:"10px 14px",background:"transparent",border:"none",color:section===n.id?COL:C.muted,fontWeight:section===n.id?700:500,fontSize:12,cursor:"pointer",borderBottom:section===n.id?`2px solid ${COL}`:"2px solid transparent",whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:5}}>
                {n.icon} {n.label}
                {(n.badge||0)>0&&<span style={{background:C.red,color:"#fff",borderRadius:"50%",width:14,height:14,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:900}}>{n.badge}</span>}
              </button>
            ))}
            <button onClick={onBack} style={{marginLeft:"auto",flexShrink:0,padding:"10px 12px",background:"transparent",border:"none",color:C.muted,fontSize:11,cursor:"pointer"}}>← {lang==="fr"?"Retour":"Back"}</button>
          </div>
        )}

        {/* Main */}
        <div style={{flex:1,padding:isMobileAdmin?"56px 14px 22px":"22px",overflowY:"auto",minWidth:0}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    <div style={{marginTop:8,display:"flex",gap:6,alignItems:"center"}}>
                      <button onClick={oracle.sync} style={{padding:"5px 12px",background:"transparent",border:`1px solid ${COL}44`,borderRadius:8,color:COL,fontSize:11,cursor:"pointer"}}>↻ Sync</button>
                      <span style={{fontSize:10,color:C.muted}}>↻ {oracle.countdown}s</span>
                    </div>
                  </div>
                </div>
...
export default function MultiversEau(){
  ...
  const PROPS={oracle,stocks,update,dec,lang,setLang,onBack:()=>{setRole(null);setShowInscRelais(false);setShowInscLiv(false);}};

  // Écrans d'inscription (accessibles depuis la landing)
  if(showInscRelais)return<InscriptionRelais {...PROPS} onSubmit={(f)=>{show(lang==="fr"?"✅ Candidature Relais envoyée !":"✅ Relay application sent!",C.green);setShowInscRelais(false);}}/>;
  if(showInscLiv)return<InscriptionLivreur {...PROPS} onSubmit={(f)=>{show(lang==="fr"?"✅ Candidature Livreur envoyée !":"✅ Driver application sent!",C.green);setShowInscLiv(false);}}/>;

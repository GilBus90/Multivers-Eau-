...
function ClientApp({oracle,stocks,dec,lang,setLang,onBack}){
  ...
  const payerAvecPi=async()=>{
    ...
    try{
      window.Pi.init({version:"2.0",sandbox:PI_SANDBOX});
      await window.Pi.authenticate(["payments"],async(inc)=>{
        try{await fetch(`/api/incomplete`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({paymentId:inc.identifier})});}catch(e){}
      });
    ...
    window.Pi.createPayment({amount:totalPi,memo:`Multivers'Eau #${orderId}`,metadata:{orderId}},{
      ...
      onReadyForServerCompletion:async(paymentId,txId)=>{try{await fetch(`/api/complete`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({paymentId,txId,orderId})});}catch(e){console.log("Complete API:",e);}finally{setPayStatus("success");Object.entries(panier).forEach(([id,qty])=>dec(id,qty));setPanier({});show(t.paiementOk,C.green);}},
